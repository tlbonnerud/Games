...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.cookies=0;//cookies (eggs)
		Game.cookiesd=0;//cookies display
		Game.cookiesPs=1;//cookies per second (to recalculate with every new purchase)
		Game.coins=0;//coins earned by selling eggs
		Game.coinsEarned=0;//lifetime coins earned
		Game.eggToCoinsRate=0.15;//base rate: 1 egg = 0.15 coins
		...
		Game.SellEggs=function(fraction)
		{
			var toSell=Math.floor(Game.cookies*fraction);
			if (toSell<1) return;
			Game.cookies-=toSell;
			var rate=Game.eggToCoinsRate;
			if (Game.Has('Markedskontrakter')) rate*=1.5;
			if (Game.Has('Meglernettverk')) rate*=1.75;
			if (Game.Has('Eksportlisens')) rate*=2;
			var earned=Math.floor(toSell*rate);
			Game.coins+=earned;
			Game.coinsEarned+=earned;
			Game.cookieNumberAdd('-'+Beautify(toSell)+' egg');
			Game.cookieNumberAdd('+'+Beautify(earned)+' coins');
		};
		...
		Game.goldenCookie.spawn=function()
		{
			...
			if ((Game.elderWrath==1 && Math.random()<0.33) || (Game.elderWrath==2 && Math.random()<0.66) || (Game.elderWrath==3))
			{
				...
				me.style.background='url(img/rottenEgg.svg)';me.style.backgroundSize='96px 96px';
			}
			else
			{
				Game.goldenCookie.wrath=0;
				me.style.background='url(img/goldenEgg.svg)';me.style.backgroundSize='96px 96px';
			}
		...
		Game.getNewTicker=function()
		{
			...
			{				
				if (Game.Objects['Kylling'].amount>0) list.push(choose([
				...
				if (Game.Objects['Kylling'].amount>=50) list.push(choose([
				...
				if (Game.Objects['Kylling'].amount>=1 && Game.pledges>0 && Game.elderWrath==0) list.push(choose([
				...
				if (Game.Objects['Hønsehus'].amount>0) list.push(choose([
				...
				if (Game.Objects['Fôrvogn'].amount>0) list.push(choose([
				...
				if (Game.Objects['Egg-sorterer'].amount>0) list.push(choose([
				...
				if (Game.Objects['Transportbånd'].amount>0) list.push(choose([
				...
				if (Game.Objects['Låvearbeider'].amount>0) list.push(choose([
				...
				if (Game.Objects['Mekanisk innhøster'].amount>0) list.push(choose([
				...
				if (Game.Objects['Robo-bonde'].amount>0) list.push(choose([
		...
		//define objects (farm units)
		// 1. Kylling — 10 egg, 0.2 egg/s
		new Game.Object('Kylling','kylling|kyllinger|lagt','En liten h\u00f8ne som legger egg jevnt og trutt.','grandma','grandmaIcon','grandmaBackground',10,function(){
			return Game.ComputeCps(0.2,Game.Has('Premium-f\u00f4r')*0.8,0);
		},Game.NewDrawFunction(function(){return 'grandma';},8,8,32,3,16),function(){
			if (this.amount>=1) Game.Unlock('Premium-f\u00f4r');
			if (this.amount>=1) Game.Win('F\u00f8rste kylling');if (this.amount>=50) Game.Win('Hensiktsfullt');if (this.amount>=100) Game.Win('H\u00f8nsegaard');
		});

		// 2. H\u00f8nsehus — 60 egg, 1.2 egg/s
		new Game.Object('H\u00f8nsehus','h\u00f8nsehus|h\u00f8nsehus|produsert','Trygt hus med bedre rutiner og h\u00f8yere eggflyt.','farm','farmIcon','farmBackground',60,function(){
			return Game.ComputeCps(1.2,Game.Has('H\u00f8nsehusdekkje')*0.5,0);
		},Game.NewDrawFunction(0,16,16,64,2,32),function(){
			if (this.amount>=2) Game.Unlock('H\u00f8nsehusdekkje');
			if (this.amount>=1) Game.Win('Hjemlig hygge');if (this.amount>=50) Game.Win('Storskala drift');
		});

		// 3. F\u00f4rvogn — 250 egg, 4 egg/s
		new Game.Object('F\u00f4rvogn','f\u00f4rvogn|f\u00f4rvogner|distribuert','Automatisk f\u00f4ring holder flokken effektiv.','factory','factoryIcon','factoryBackground',250,function(){
			return Game.ComputeCps(4,Game.Has('F\u00f4rvogn-turbo')*0.85,0);
		},Game.NewDrawFunction(0,32,2,64,1,-22),function(){
			if (this.amount>=4) Game.Unlock('F\u00f4rvogn-turbo');
			if (this.amount>=1) Game.Win('Automatisert f\u00f4ring');if (this.amount>=50) Game.Win('F\u00f4rfabrikk');
		});

		// 4. Egg-sorterer — 900 egg, 12 egg/s
		new Game.Object('Egg-sorterer','egg-sorterer|egg-sorterere|sortert','Sorterer og pakker egg med minimalt svinn.','mine','mineIcon','mineBackground',900,function(){
			return Game.ComputeCps(12,Game.Has('Sorteringsalgoritme')*0.75,0);
		},Game.NewDrawFunction(0,16,16,64,2,24),function(){
			if (this.amount>=2) Game.Unlock('Sorteringsalgoritme');
			if (this.amount>=1) Game.Win('Logistikkekspert');if (this.amount>=50) Game.Win('Sorteringskjempe');
		});

		// 5. Transportb\u00e5nd — 3000 egg, 35 egg/s
		new Game.Object('Transportb\u00e5nd','transportb\u00e5nd|transportb\u00e5nd|fraktet','Kontinuerlig logistikk mellom hus, lager og marked.','shipment','shipmentIcon','shipmentBackground',3000,function(){
			return Game.ComputeCps(35,Game.Has('Smurte kjeder')*0.7,0);
		},Game.NewDrawFunction(0,16,16,64),function(){
			if (this.amount>=1) Game.Unlock('Smurte kjeder');
			if (this.amount>=1) Game.Win('P\u00e5 l\u00f8pende b\u00e5nd');if (this.amount>=50) Game.Win('Masselogistikk');
		});

		// 6. L\u00e5vearbeider — 9000 egg, 90 egg/s
		new Game.Object('L\u00e5vearbeider','l\u00e5vearbeider|l\u00e5vearbeidere','Menneskelig kapasitet gir bedre flyt og kontroll.','alchemylab','alchemylabIcon','alchemylabBackground',9000,function(){
			return Game.ComputeCps(90,Game.Has('Ergonomisk sele')*0.9,0);
		},Game.NewDrawFunction(0,16,16,64,2,16),function(){
			if (this.amount>=4) Game.Unlock('Ergonomisk sele');
			if (this.amount>=1) Game.Win('Gode hender');if (this.amount>=50) Game.Win('Barnegaard');
		});

		// 7. Mekanisk innh\u00f8ster — 28000 egg, 250 egg/s
		new Game.Object('Mekanisk innh\u00f8ster','mekanisk innh\u00f8ster|mekaniske innh\u00f8stere','Tung maskin som l\u00f8fter produksjonen kraftig.','portal','portalIcon','portalBackground',28000,function(){
			return Game.ComputeCps(250,Game.Has('Hydraulisk arm')*1,0);
		},Game.NewDrawFunction(0,32,32,64,2),function(){
			if (this.amount>=3) Game.Unlock('Hydraulisk arm');
			if (this.amount>=1) Game.Win('Mekanikerdr\u00f8mmen');if (this.amount>=50) Game.Win('Industriell kraft');
		});

		// 8. Robo-bonde — 92000 egg, 700 egg/s
		new Game.Object('Robo-bonde','robo-bonde|robo-b\u00f8nder','Autonom superenhet for ekte fabrikktempo.','timemachine','timemachineIcon','timemachineBackground',92000,function(){
			return Game.ComputeCps(700,Game.Has('Kvantechipen')*1.2,0);
		},Game.NewDrawFunction(0,32,32,64,1),function(){
			if (this.amount>=2) Game.Unlock('Kvantechipen');
			if (this.amount>=1) Game.Win('Fremtiden er n\u00e5');if (this.amount>=50) Game.Win('Autonomi');
		});
		...
		order=200;
		new Game.Upgrade('Forwards from grandma','Grandmas gain <b>+0.3</b> base CpS.<q>RE:RE:thought you\'d get a kick out of this ;))</q>',Game.Objects['Kylling'].basePrice*tier1,[1,0]);
		new Game.Upgrade('Steel-plated rolling pins','Grandmas are <b>twice</b> as efficient.',Game.Objects['Kylling'].basePrice*tier2,[1,0]);
		new Game.Upgrade('Lubricated dentures','Grandmas are <b>twice</b> as efficient.<q>Squish</q>',Game.Objects['Kylling'].basePrice*tier3,[1,1]);
		
		order=300;
		new Game.Upgrade('Cheap hoes','Farms gain <b>+0.5</b> base CpS.',Game.Objects['Hønsehus'].basePrice*tier1,[2,0]);
		new Game.Upgrade('Fertilizer','Farms are <b>twice</b> as efficient.<q>It\'s chocolate, I swear.</q>',Game.Objects['Hønsehus'].basePrice*tier2,[2,0]);
		new Game.Upgrade('Cookie trees','Farms are <b>twice</b> as efficient.<q>A relative of the breadfruit.</q>',Game.Objects['Hønsehus'].basePrice*tier3,[2,1]);
		
		order=400;
		new Game.Upgrade('Sturdier conveyor belts','Factories gain <b>+4</b> base CpS.',Game.Objects['Fôrvogn'].basePrice*tier1,[4,0]);
		new Game.Upgrade('Child labor','Factories are <b>twice</b> as efficient.<q>Cheaper, healthier workforce - and so much more receptive to whipping!</q>',Game.Objects['Fôrvogn'].basePrice*tier2,[4,0]);
		new Game.Upgrade('Sweatshop','Factories are <b>twice</b> as efficient.<q>Slackers will be terminated.</q>',Game.Objects['Fôrvogn'].basePrice*tier3,[4,1]);
		
		order=500;
		new Game.Upgrade('Sugar gas','Mines gain <b>+10</b> base CpS.<q>A pink, volatile gas, found in the depths of some chocolate caves.</q>',Game.Objects['Egg-sorterer'].basePrice*tier1,[3,0]);
		new Game.Upgrade('Megadrill','Mines are <b>twice</b> as efficient.',Game.Objects['Egg-sorterer'].basePrice*tier2,[3,0]);
		new Game.Upgrade('Ultradrill','Mines are <b>twice</b> as efficient.',Game.Objects['Egg-sorterer'].basePrice*tier3,[3,1]);
		
		order=600;
		new Game.Upgrade('Vanilla nebulae','Shipments gain <b>+30</b> base CpS.',Game.Objects['Transportbånd'].basePrice*tier1,[5,0]);
		new Game.Upgrade('Wormholes','Shipments are <b>twice</b> as efficient.<q>By using these as shortcuts, your ships can travel much faster.</q>',Game.Objects['Transportbånd'].basePrice*tier2,[5,0]);
		new Game.Upgrade('Frequent flyer','Shipments are <b>twice</b> as efficient.<q>Come back soon!</q>',Game.Objects['Transportbånd'].basePrice*tier3,[5,1]);
		
		order=700;
		new Game.Upgrade('Antimony','Alchemy labs gain <b>+100</b> base CpS.<q>Actually worth a lot of mony.</q>',Game.Objects['Låvearbeider'].basePrice*tier1,[6,0]);
		new Game.Upgrade('Essence of dough','Alchemy labs are <b>twice</b> as efficient.<q>Extracted through the 5 ancient steps of alchemical baking.</q>',Game.Objects['Låvearbeider'].basePrice*tier2,[6,0]);
		new Game.Upgrade('True chocolate','Alchemy labs are <b>twice</b> as efficient.<q>The purest form of cacao.</q>',Game.Objects['Låvearbeider'].basePrice*tier3,[6,1]);
		
		order=800;
		new Game.Upgrade('Ancient tablet','Portals gain <b>+1,666</b> base CpS.<q>A strange slab of peanut brittle, holding an ancient cookie recipe. Neat!</q>',Game.Objects['Mekanisk innhøster'].basePrice*tier1,[7,0]);
		new Game.Upgrade('Insane oatling workers','Portals are <b>twice</b> as efficient.<q>ARISE, MY MINIONS!</q>',Game.Objects['Mekanisk innhøster'].basePrice*tier2,[7,0]);
		new Game.Upgrade('Soul bond','Portals are <b>twice</b> as efficient.<q>So I just sign up and get more cookies? Sure, whatever!</q>',Game.Objects['Mekanisk innhøster'].basePrice*tier3,[7,1]);
		...
		order=200;new Game.Upgrade('Prune juice','Grandmas are <b>twice</b> as efficient.<q>Gets me going.</q>',Game.Objects['Kylling'].basePrice*tier4,[1,2]);
		order=300;new Game.Upgrade('Genetically-modified cookies','Farms are <b>twice</b> as efficient.<q>All-natural mutations.</q>',Game.Objects['Hønsehus'].basePrice*tier4,[2,2]);
		order=400;new Game.Upgrade('Radium reactors','Factories are <b>twice</b> as efficient.<q>Gives your cookies a healthy glow.</q>',Game.Objects['Fôrvogn'].basePrice*tier4,[4,2]);
		order=500;new Game.Upgrade('Ultimadrill','Mines are <b>twice</b> as efficient.<q>Pierce the heavens, etc.</q>',Game.Objects['Egg-sorterer'].basePrice*tier4,[3,2]);
		order=600;new Game.Upgrade('Warp drive','Shipments are <b>twice</b> as efficient.',Game.Objects['Transportbånd'].basePrice*tier4,[5,2]);
		order=700;new Game.Upgrade('Ambrosia','Alchemy labs are <b>twice</b> as efficient.',Game.Objects['Låvearbeider'].basePrice*tier4,[6,2]);
		order=800;new Game.Upgrade('Sanity dance','Portals are <b>twice</b> as efficient.<q>We can change if we want to.<br>We can leave our brains behind.</q>',Game.Objects['Mekanisk innhøster'].basePrice*tier4,[7,2]);
		...
		order=250;
		new Game.Upgrade('Farmer grandmas','Grandmas are <b>twice</b> as efficient.',Game.Objects['Hønsehus'].basePrice*tier2,[10,9],function(){Game.Objects['Kylling'].drawFunction();});
		new Game.Upgrade('Worker grandmas','Grandmas are <b>twice</b> as efficient.',Game.Objects['Fôrvogn'].basePrice*tier2,[10,9],function(){Game.Objects['Kylling'].drawFunction();});
		new Game.Upgrade('Miner grandmas','Grandmas are <b>twice</b> as efficient.',Game.Objects['Egg-sorterer'].basePrice*tier2,[10,9],function(){Game.Objects['Kylling'].drawFunction();});
		new Game.Upgrade('Cosmic grandmas','Grandmas are <b>twice</b> as efficient.',Game.Objects['Transportbånd'].basePrice*tier2,[10,9],function(){Game.Objects['Kylling'].drawFunction();});
		new Game.Upgrade('Transmuted grandmas','Grandmas are <b>twice</b> as efficient.',Game.Objects['Låvearbeider'].basePrice*tier2,[10,9],function(){Game.Objects['Kylling'].drawFunction();});
		new Game.Upgrade('Altered grandmas','Grandmas are <b>twice</b> as efficient.',Game.Objects['Mekanisk innhøster'].basePrice*tier2,[10,9],function(){Game.Objects['Kylling'].drawFunction();});
		new Game.Upgrade('Grandmas\' grandmas','Grandmas are <b>twice</b> as efficient.',Game.Objects['Robo-bonde'].basePrice*tier2,[10,9],function(){Game.Objects['Kylling'].drawFunction();});
		...
		order=1000;
		new Game.Upgrade('Sugar bosons','Antimatter condensers gain <b>+99,999</b> base CpS.',Game.Objects['Robo-bonde'].basePrice*tier1,[13,0]);
		new Game.Upgrade('String theory','Antimatter condensers are <b>twice</b> as efficient.',Game.Objects['Robo-bonde'].basePrice*tier2,[13,0]);
		new Game.Upgrade('Large macaron collider','Antimatter condensers are <b>twice</b> as efficient.<q>How singular!</q>',Game.Objects['Robo-bonde'].basePrice*tier3,[13,1]);
		new Game.Upgrade('Big bang bake','Antimatter condensers are <b>twice</b> as efficient.<q>And that\'s how it all began.</q>',Game.Objects['Robo-bonde'].basePrice*tier4,[13,2]);

		order=250;
		new Game.Upgrade('Antigrandmas','Grandmas are <b>twice</b> as efficient.',Game.Objects['Robo-bonde'].basePrice*tier2,[10,9],function(){Game.Objects['Kylling'].drawFunction();});
		...
		// Farm-spesifikke oppgraderinger
		order=50000;
		new Game.Upgrade('Premium-f\u00f4r','Kyllingene er <b>1.8\u00d7</b> s\u00e5 effektive.<q>Eksklusiv blandingskost for h\u00f8ye ytelser.</q>',900,[1,0]);
		new Game.Upgrade('H\u00f8nsehusdekkje','H\u00f8nsehus er <b>1.5\u00d7</b> s\u00e5 effektive.<q>Varmeisolasjon og god ventilasjon.</q>',480,[2,0]);
		new Game.Upgrade('F\u00f4rvogn-turbo','F\u00f4rvogner er <b>1.85\u00d7</b> s\u00e5 effektive.<q>Elektrisk motor, r\u00e5fort.</q>',5200,[4,0]);
		new Game.Upgrade('Sorteringsalgoritme','Egg-sorterere er <b>1.75\u00d7</b> s\u00e5 effektive.<q>Maskinl\u00e6ring for perfekt eggklassifisering.</q>',2400,[3,0]);
		new Game.Upgrade('Smurte kjeder','Transportb\u00e5nd er <b>1.7\u00d7</b> s\u00e5 effektive.<q>Null friksjon, maks gjennomstr\u00f8mming.</q>',6800,[5,0]);
		new Game.Upgrade('Ergonomisk sele','L\u00e5vearbeidere er <b>1.9\u00d7</b> s\u00e5 effektive.<q>HMS-godkjent, selvf\u00f8lgelig.</q>',22000,[6,0]);
		new Game.Upgrade('Hydraulisk arm','Mekaniske innh\u00f8stere er <b>2\u00d7</b> s\u00e5 effektive.<q>L\u00f8fter tungt uten \u00e5 bremse ned.</q>',48000,[7,0]);
		new Game.Upgrade('Kvantechipen','Robo-b\u00f8nder er <b>2.2\u00d7</b> s\u00e5 effektive.<q>Kvante-CPU for autonome avveiinger.</q>',120000,[8,0]);

		// Coins-oppgraderinger
		new Game.Upgrade('Markedskontrakter','Egg selges for <b>1.5\u00d7</b> s\u00e5 mange coins.<q>Forhndlingsfortrinn i markedet.</q>',1200,[1,1]);
		new Game.Upgrade('Meglernettverk','Egg selges for <b>1.75\u00d7</b> s\u00e5 mange coins.<q>B\u00f8rsnotert egg-megler.</q>',7600,[2,1]);
		new Game.Upgrade('Eksportlisens','Egg selges for <b>2\u00d7</b> s\u00e5 mange coins.<q>Internasjonalt eggmarked.</q>',14000,[3,1]);
		...
		*/
		...
		{
			if (Game.Has('Elder Covenant') || Game.Objects['Kylling'].amount==0) Game.elderWrath=0;
	...
	Game.Logic=function()
	{
		...
		if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
		{
			...
			//if (Game.Objects['Fôrvogn'].amount>=50 && Game.Objects['Fôrvogn'].specialUnlocked==0) {Game.Objects['Fôrvogn'].unlockSpecial();Game.Popup('You have unlocked the factory dungeons!');}
			...
			if (Game.Objects['Kylling'].amount>=6 && !Game.Has('Bingo center/Research facility') && Game.HasAchiev('Elder')) Game.Unlock('Bingo center/Research facility');
	...
	Game.Draw=function()
	{
		...
		if (Game.prefs.particles)
		{
			var amount=Game.Objects['Kylling']?Game.Objects['Kylling'].amount:0;
